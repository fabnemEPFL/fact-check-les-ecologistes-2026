...
def main():
    ...
    manifest={"generated_at_utc":datetime.now(timezone.utc).isoformat(),"checkpoint_factuel_utilise":ref["name"],"sha256_checkpoint":hashlib.sha256(cp_path.read_bytes()).hexdigest(),"checkpoint_uncompressed_sha256":ref["uncompressed_sha256"],"statistiques_finales":{v:counts[v] for v in ALLOWED},"estimation_energetique_kwh":{"central":energy["summary"]["central_kwh"],"low":energy["summary"]["low_kwh"],"high":energy["summary"]["high_kwh"]},"pdf":{"name":PDF.name,"size_bytes":PDF.stat().st_size,"pages":len(reader.pages),"sha256":sha,"link_annotations":links},"source":{"name":MD.name,"sha256":hashlib.sha256(MD.read_bytes()).hexdigest()},"qa":{"ids":len(ids),"rendered_pages":len(imgs),"status":"passed"}}
